...
class PresetGraph:
    ...
    def randomly_rewire(self) -> GraphData:
        graphData= self.graph_data
        rewired_graph = gt.Graph(graphData.edge_list,directed=False)
        gt.random_rewire(rewired_graph,model='configuration')
        return GraphData(vertex_set = graphData.vertex_set,weights = graphData.weights,
                         edge_list = list(rewired_graph.get_edges()))
