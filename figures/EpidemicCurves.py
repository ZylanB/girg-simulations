...
def plot_psi_inset(curve_data: EpidemicCurveData, infection_range: Tuple[int, int]):
    ...
    plt.plot(time_coords, infection_coords, linewidth=3, color=colours.BLUE)
    plt.plot(time_coords, [10 ** x for x in regression_infection_coords], linewidth=6, color=colours.RED,
             linestyle="dashed")
...
def generate_figures():
    ...
    for i in range(len(CURVE_PARAMS)):
        ...
        data = generate_data(CURVE_PARAMS[i], seed=SEEDS[i], name=f"{FILENAME_BASE}{i}")
