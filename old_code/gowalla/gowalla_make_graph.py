...
path_checkins = ''
path_edges = '/home/zylan/python/gowalla/Gowalla_edges.txt'
...
with open('/home/zylan/python/gowalla/gow_graph_mode.pickle','rb') as data:
    z,x,c,gow_pos_dict = pickle.load(data)
gow_edges_txt = open(path_edges,'r')
gow_edges_string = gow_edges_txt.readlines()
print(len(gow_edges_string))
gow_edge_list= []
edge_dict_dummy = {}
k = 0
...
for x in gow_edges_string:
    ...
    a = int(x.split('\t')[0])
    b = int(x.split('\t')[1])
    if a in gow_pos_dict.keys() and b in gow_pos_dict.keys() and (a,b) not in edge_dict_dummy.keys() and (b,a) not in edge_dict_dummy.keys():
        gow_edge_list.append((a,b))
        edge_dict_dummy[(a,b)] = 0
gow_edges_txt.close()
print("1")
...
breakpoint()
with open('gow_graph_mode_new.pickle','wb') as outfile:
