...
def genLattice(lim,d,tau,alpha, deg = None, seed = None):
    ...
    edges = gs.generateEdges(weights_scaled,nodes,alpha,scale=1/((lim+1)**2), seed = seed)
...
def genGirg(poisson_n,d,tau,alpha,deg = None, seed = None):
    ...
    origin_node = [0.50]*d
    nodes = np.insert(nodes,0,origin_node,axis=0)#nodes = np.insert(nodes,0,[0.50,0.50],axis=0)
    weights = np.asarray(gs.generateWeights(n+1,ple=tau, seed = seed))
    weights_scaled = weights
    if deg is not None:
        beta = gs.scaleWeights(weights,deg,d,alpha)
        weights_scaled = weights*beta
    edges = gs.generateEdges(weights_scaled,nodes,alpha,scale=1/(2*n), seed = seed)
    nodes = nodes*box_d
...
def infectionSpread(g,status,weights,L_rv,mu, zeta = 0,vertex_set = "Z2", ratio = 1, origin_index = None, method = 4, penalize = False, d=2, pos = []):
    start_time = time.time()
    ...
    for e in g.edges():
        match vertex_set:
            ...
            case "PPP":
                lim = g.num_vertices()**(1/d)
                coord_sum = 0
                for i in range(d):
                    coord_sum += (min(abs(pos[e.source()][i] - pos[e.target()][i]), lim - abs(pos[e.source()][i] - pos[e.target()][i])))**d
                distance = coord_sum**(1/d)
                #distance = max(min(abs(pos[e.source()][0] - pos[e.target()][0]), lim - abs(pos[e.source()][0] - pos[e.target()][0])), min(abs(pos[e.source()][1] - pos[e.target()][1]), lim - abs(pos[e.source()][1] - pos[e.target()][1]))) 
        match method:
            case 1:
...
def epidemic_curves(size,alpha,tau,params,num_runs,file_marker):
    ...
    for mu,zeta,label in params:
        ...
        if label == "polynomial" or label == "pure geometric":
            ...
            lr_start = np.where(y_lr>=3)[0][0] 
            lr_end = np.where(y_lr>=5)[0][0]
...
def typicalCostDistance(g,pos,infs,noinfecs,origin_index,r):
    cost_distances = []
    lim = sqrt(g.num_vertices())
    for v in g.iter_vertices():
        distance = max(min(abs(pos[origin_index][0] - pos[v][0]), lim - abs(pos[origin_index][0] - pos[v][0])), min(abs(pos[origin_index][1] - pos[v][1]), lim - abs(pos[origin_index][1] - pos[v][1])))
        if abs(distance-r) <= 0.5  and v not in noinfecs:
            cost_distances.append(infs[v][1])
    return np.mean(cost_distances)

def typicalCostDistance1D(g,pos,infs,noinfecs,origin_index,r,d):
    cost_distances = []
    lim = (g.num_vertices())**(1/d)
    dists=[]
    for v in g.iter_vertices():
        coord_list = []
        for i in range(d):
            coord_list.append(min(abs(pos[origin_index][i] - pos[v][i]), lim - abs(pos[origin_index][i] - pos[v][i])))
        distance = max(coord_list)
        dists.append(distance)
        if abs(distance-r) <= 10  and v not in noinfecs:
            cost_distances.append(infs[v][1])
    return np.mean(cost_distances)

def edgeCostBetweenBoxes(g,st,weights,pos,L_rv,d,mu,zeta,gamma):
    lim = g.num_vertices()**(1/d)
    infs,tc,noninfecs = infectionSpread(g,st,weights,L_rv,mu=mu,zeta=zeta,vertex_set="PPP",ratio = 0.01,d=d,pos=pos)
    r = 5
    filter = g.new_vertex_property("bool")
    for u in g.iter_vertices():
        sum_coords = 0
        for i in range(d):
            sum_coords += (min(abs(pos[0][i] - pos[int(u)][i]), lim - abs(pos[0][i] - pos[int(u)][i])))**d
        distance = sum_coords**(1/d)
        if d == 1:
            coef = 0.4
        if d == 2:
            coef = 0.7
        if abs(distance - coef*lim) <= r:
            target_vertex = u
            break
    L_dist = 0
    for i in range(d):
        L_dist += (min(abs(pos[0][i] - pos[target_vertex][i]), lim - abs(pos[0][i] - pos[target_vertex][i])))**d
    L_dist = L_dist**(1/d)
    box_size = L_dist**gamma
    vertices = {}
    origin_max_dist = box_size/2 #sqrt(2*min(box_size/2,lim-box_size/2)**2)
    target_max_dist = box_size/2 #sqrt(min(abs(pos[target_vertex][0]- (pos[target_vertex][0]+box_size/2)),lim-abs(pos[target_vertex][0]- (pos[target_vertex][0]+box_size/2)))**2 + min(abs(pos[target_vertex][1]- (pos[target_vertex][1]+box_size/2)),lim-abs(pos[target_vertex][1]- (pos[target_vertex][1]+box_size/2)))**2)
    for u in g.iter_vertices():
        origin_coords = 0
        target_coords = 0
        for i in range(d):
            origin_coords += (min(abs(pos[0][i] - pos[int(u)][i]), lim - abs(pos[0][i] - pos[int(u)][i])))**d
            target_coords += (min(abs(pos[target_vertex][i] - pos[int(u)][i]), lim - abs(pos[target_vertex][i] - pos[int(u)][i])))**d
        origin_distance = origin_coords**(1/d)
        target_distance = target_coords**(1/d)
        if origin_distance <= origin_max_dist:
            vertices[u] = "origin"
            filter[u] = True
        if target_distance <= target_max_dist:
            vertices[u] = "target"
            filter[u] = True
        if origin_distance <= origin_max_dist and target_distance <= target_max_dist:
            print("ayo")
            breakpoint()
    ...
    for e in g.iter_edges():
        if (vertices[e[1]] == "target" and vertices[e[0]] == "origin") or (vertices[e[1]] == "origin" and vertices[e[0]] == "target"):
            if tc[e] < min_cost:
                min_cost = tc[e]
                dist = 0 
                for i in range(d):
                    dist += (min(abs(pos[e[0]][i] - pos[e[1]][i]), lim - abs(pos[e[0]][i] - pos[e[1]][i])))**d
                dist = dist**(1/d)
                est_eta = np.log(min_cost)/np.log(dist)
                

    print("Cost Simulation: --- %s seconds ---" % (time.time() - start_time))
    return est_eta
